...
import { Upload, FileText, Trash2, Loader2, Download, MessageSquareText, Trophy } from "lucide-react";
...
export default function App() {
  ...
  // Helpers
  const isDuplicateFile = (newFile: File) => resumeFiles.some(f => f.file.name === newFile.name && f.file.size === newFile.size);
  ...
  const handleResumeFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of newFilesToAdd) {
      ...
      try {
        const response = await fetch(`${BACKEND_URL}/upload_resume/`, { method: "POST", body: formData });
        if (!response.ok) {
    ...
    if (successfulUploads > 0) setResumeFiles((prev) => [...prev, ...uploadedResumeDetails]);
  ...
  const handleRemoveResume = (id: string) => {
    setResumeFiles((prev) => prev.filter((resume) => resume.id !== id));
  ...
  const handleRankResumes = async () => {
    if (!jobDescriptionFile) return toast.error("Upload a JD PDF first!");
    if (resumeFiles.length === 0) return toast.error("Upload at least one resume!");
    setIsRanking(true); setRankedResumes([]); setJobDescriptionText("");
    const formData = new FormData();
    formData.append("jd_file", jobDescriptionFile);
    try {
      const response = await fetch(`${BACKEND_URL}/rank_resumes/`, { method: "POST", body: formData });
      if (!response.ok) {
        const errorData = await response.json();
      ...
      toast.success("Resumes ranked!");
    } catch (error: any) {
      toast.error(`Ranking error: ${error.message}`);
    } finally {
      setIsRanking(false);
    }
  };
  ...
  const handleGetLLMFeedback = async (resume: RankedResume) => {
    ...
    if (!jobDescriptionText) return toast.error("JD text not available. Please rank resumes first.");
    ...
    try {
      ...
      toast.error(`Feedback error: ${error.message}`);
    } finally {
  ...
  const toggleFeedbackVisibility = (resumeId: string) => {
    setRankedResumes((prev) =>
      prev.map((r) => r.resume_id === resumeId ? { ...r, showFeedback: !r.showFeedback } : r)
    );
  };

  // --- UI ---
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-sky-50 via-indigo-50 to-fuchsia-100 font-inter flex flex-col">
      ...
      <motion.h1
        ...
        className="text-4xl sm:text-5xl font-extrabold text-center my-12 drop-shadow-lg bg-gradient-to-r from-blue-600 via-purple-600 to-fuchsia-500 bg-clip-text text-transparent"
      >
        <Trophy className="inline-block w-10 h-10 text-yellow-400 mr-2" /> Semantic Resume Ranker
      </motion.h1>

      <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 mb-12">
        {/* JD UPLOAD */}
        <motion.div
          initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
          className="bg-white bg-opacity-80 backdrop-blur-lg border border-blue-200 p-8 rounded-3xl shadow-2xl hover:shadow-blue-200 transition-shadow"
        >
          ...
          <label
            htmlFor="jd-upload"
            className="flex flex-col items-center justify-center w-full h-36 border-2 border-dashed border-blue-400 rounded-lg cursor-pointer bg-blue-50 hover:bg-blue-100 transition-colors duration-300"
            tabIndex={0} aria-label="Upload Job Description"
          >
            ...
            <input
              id="jd-upload" type="file" className="hidden" accept=".pdf"
              onChange={handleJobDescriptionFileChange} ref={jdFileInputRef}
            />
          </label>
          <AnimatePresence>
            {jobDescriptionFile && (
        ...
        <motion.div
          ...
          className="bg-white bg-opacity-80 backdrop-blur-lg border border-purple-200 p-8 rounded-3xl shadow-2xl hover:shadow-purple-200 transition-shadow"
        >
          ...
          <label
            ...
            className="flex flex-col items-center justify-center w-full h-36 border-2 border-dashed border-purple-400 rounded-lg cursor-pointer bg-purple-50 hover:bg-purple-100 transition-colors duration-300"
            tabIndex={0} aria-label="Upload Resumes"
          >
            <div className="flex flex-col items-center justify-center pt-5 pb-6">
            ...
            <input
              id="resume-upload" type="file" className="hidden" accept=".pdf" multiple
              onChange={handleResumeFileChange} ref={resumeFileInputRef}
            />
          </label>
          {isUploadingResumes && (
            ...
            <div className="mt-4 max-h-60 overflow-y-auto pr-2">
              <AnimatePresence mode="popLayout">
                {resumeFiles.map((resume) => (
                  <motion.div
                    key={resume.id}
                    initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} layout
      ...
      <AnimatePresence>
        {rankedResumes.length > 0 ? (
          ...
          >
            ...
            <p className="text-center text-gray-500 mb-8 text-base">Higher scores mean your resume matches the job description better. Lower scores mean your resume may not align with the job requirements as closely. Click "Get LLM Feedback" for tailored improvement tips!</p>
